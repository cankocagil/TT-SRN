...
ReLU activations are replaced with GELUs. 
"""
import io
...
class TTSPNsegm(nn.Module):
    def __init__(self, ttspn, freeze_ttspn=False):
        super().__init__()
        self.ttspn = ttspn
        ...
        hidden_dim, nheads = ttspn.transformer.d_model, ttspn.transformer.nhead
    ...
    def forward(self, samples: NestedTensor):
        ...
        features, pos = self.ttspn.backbone(samples)
        bs = features[-1].tensors.shape[0]
        src, mask = features[-1].decompose()
        assert mask is not None
        src_proj = self.ttspn.input_proj(src)
        n,c,s_h,s_w = src_proj.shape
        bs_f = bs//self.ttspn.num_frames
        src_proj = src_proj.reshape(bs_f, self.ttspn.num_frames,c, s_h, s_w).permute(0,2,1,3,4).flatten(-2)
        mask = mask.reshape(bs_f, self.ttspn.num_frames, s_h*s_w)
        pos = pos[-1].permute(0,2,1,3,4).flatten(-2)
        hs, memory = self.ttspn.transformer(src_proj, mask, self.ttspn.query_embed.weight, pos)
        outputs_class = self.ttspn.class_embed(hs)
        outputs_coord = self.ttspn.bbox_embed(hs).sigmoid()
        out = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord[-1]}
        if self.ttspn.aux_loss:
            out['aux_outputs'] = self.ttspn._set_aux_loss(outputs_class, outputs_coord)
        for i in range(3):
            _,c_f,h,w = features[i].tensors.shape
            features[i].tensors = features[i].tensors.reshape(bs_f, self.ttspn.num_frames, c_f, h,w)
        n_f = self.ttspn.num_queries//self.ttspn.num_frames
        outputs_seg_masks = []
        
        # image level processing using box attention
        for i in range(self.ttspn.num_frames):
